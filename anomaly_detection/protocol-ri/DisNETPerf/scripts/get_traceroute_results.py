...
import datetime
import time
import DisNETPerf.scripts.AUX_IP_to_AS_map as parseIP
import DisNETPerf.scripts.AUX_get_RouteViews_data as rv
...
class TracerouteMeasurement:
    ...
    def __init__(self):
        ...
        self.timestamp = '' # the UNIX-timestamp corresponding to the time at which the measurement was launched
    ...
    def saveToFile(self, ASMapping, currentTime):
        ...
        """
        try:
            pointerToFile = open('DisNETPerf/output/' + currentTime + '_scheduled_traceroutes.txt', 'a', 0)
        except IOError:
            print("error: Could not open/create file 'DisNETPerf/output/" + currentTime + "'_scheduled_traceroutes.txt'\n")
            return None
...
if __name__ == '__main__':
    ...
    if loadIPToPoPMapping('DisNETPerf/lib/ip_to_pop_mapping.txt') == None:
        exit(2)

    retrieve_traceroute_results('DisNETPerf/input/' + arguments["filename"], True)