 ...
 * 
 */
public class ClosedSortingMapper extends
        Mapper<Text, TopKStringPatterns, LongWritable, Text> {
    ...
    protected void map(Text key, TopKStringPatterns values, Context context)
        ...
        for (Pair<List<String>, Long> pattern : values.getPatterns()) {
            int support = pattern.getSecond().intValue();

            context.write(
                    new LongWritable(-support),
                    new Text(pattern.getFirst()
                            + "\t"
                            + support
                            + " - "
                            + String.format("%.3f",
