...
def services(repository_ip, repository_rrd_port, repository_streaming_port, repository_log_port, config_path):
    ...
    if (repository_ip is not None and
        ...
        services.append(RepositoryService(compute_caching_performance()))
    else:
...
def compute_caching_performance():
    cap = mplane.model.Capability(label="repository-caching_performance", when = "now ... future")
    cap.add_metadata("System_type", "repository")
    cap.add_metadata("System_ID", "repository-Proxy")
    cap.add_metadata("System_version", "0.1")
...
class RepositoryService(mplane.scheduler.Service):
    ...
    def run(self, spec, check_interrupt):
        ...
        elif ("repository-caching_performance" in spec.get_label()):
            print(spec.get_label(), spec.get_parameter_value("cache.size"),
            spec.get_parameter_value("cache.algo"),
            spec.get_parameter_value("cache.bins"))
    ...
    def change_conf(self, cap_label, enable):
        ...
        if enable == True:
            ...
            elif "repository-caching_performance" in cap_label:
                print("repository-caching_performance Enabled \n")
            else:
            ...
            if "repository-collect_rrd" in cap_label :
                ...
                print("repository-caching_performance Disabled \n")
            else:
                print("UNKNOWN CAPABILITY \n")
